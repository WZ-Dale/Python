...
class Detection:
    def __init__(self, image, zuidashang):
        ...
        self.ui.label_5.setText("检测日期：" + str(time.strftime("%Y-%m-%d", time.localtime(time.time()))))
        self.ui.pushButton.clicked.connect(self.show_graph2)
        self.ui.pushButton.clicked.connect(self.crack_detect)
        ...
        self.ms.show_update.connect(self.update_graph2)    # 绑定自定义信号的处理函数
        self.ms.text_print.connect(self.printToGui)
        self.ms.res_sign.connect(self.show_ticket)
